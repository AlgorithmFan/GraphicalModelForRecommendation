...
'''
https://github.com/dawenl/stochastic_PMF/blob/master/code/pmf.py
'''
...
class PoissonMF(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_components=100, max_iter=100, tol=0.0005, smoothness=100, random_state=None, verbose=False, **kwargs):
        ...
        Augument:
            ...
            smoothness: int
                smoothness on the initialization variantional parameters
    ...
    def _init_component(self, n_feats):
        # variational parameters for beta
        self.gamma_b = self.smoothness * np.random.gamma(self.smoothness, 1./self.smoothness, size=(self.n_components, n_feats))
        self.rho_b = self.smoothness * np.random.gamma(self.smoothness, 1./self.smoothness, size=(self.n_components, n_feats))
        self.Eb, self.Elogb = _compute_expectations(self.gamma_b, self.rho_b)

    def set_component(self, shape, rate):
...
class OnlinePoissonMF(PoissonMF):
    ...
    def __init__(self, n_components=100, batch_size=10, n_pass=10, max_iter=100, tol=0.0005, shuffle=True, smoothness=100, random_state=None, verbose=False, **kwargs):
        ''' Poisson matrix factorization
        Argument
        --------------
        n_components : int
    ...
    def partial_fit(self, X):
        ''' Fit the data in X as a mini-batch and update the parameter by taking a natural gradient step. Could be invoked from a high-level out-of-core wrapper.

        Parameters
        -------------
        X : array-like, shape (batch_size, n_fedats)
        Mini-batch data.
        ...
        ratio = X / self._xexplog()
        self.gamma_b = (1 - self.rho) * self.gamma_b + self.rho * (self.b + self._scale * np.exp(self.Elogb) * np.dot(np.exp(self.Elogt).T, ratio))
        self.rho_b = (1 - self.rho) * self.rho_b + self.rho * (self.b + self._scale * np.sum(self.Et, axis=0, keepdims=True).T)
        self.Eb, self.Elogb = _compute_expectations(self.gamma_b, self.rho_b)
        return self
    ...
    def _stoch_bound(self, X):
        ...
        bound *= self._scale
        bound += _gamma_term(self.b, self.b, self.gamm_b, self.rho_b, self.Eb, self.Elogb)
        return bound
